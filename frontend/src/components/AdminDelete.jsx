...
const AdminDelete = () => {
  ...
  const handleDelete = async (id) => {
    ...
    } catch (err) { setError('Failed to delete problem'); console.error(err); }
  };
  ...
  if (error) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'rgba(255,69,96,0.1)', border: '1px solid rgba(255,69,96,0.25)', borderRadius: '10px', padding: '20px 28px', color: 'var(--accent-red)', fontFamily: 'Space Mono, monospace', fontSize: '14px' }}>{error}</div>
    </div>
  );
